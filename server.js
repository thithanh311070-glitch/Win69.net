...
io.on("connection", (socket) => {
  ...
  socket.on("slot", (amount) => {
    if (players[socket.id].money < amount) return;
    players[socket.id].money -= amount;
  ...
  socket.on("guess_number", ({number, amount}) => {
    ...
    let win = number === lucky;

    if (win) players[socket.id].money += amount*3;

    socket.emit("guess_result",{lucky,win});
  });
...
setInterval(()=>{
  ...
  io.emit("taixiu_result",{dice,total,result,jackpot,players});
  bets=[];
